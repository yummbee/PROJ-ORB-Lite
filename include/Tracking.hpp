...
enum TrackingState { NO_IMAGES_YET, NOT_INITIALIZED, OK, LOST };
...
struct Tracking {
  ...
  Vec3 p_prev_kf = {0, 0, 0};
  Quaternion q_prev_kf = {1, 0, 0, 0};
  int last_kf_frame = 0;
  ...
  bool track(const Image &img, const std::vector<ImuSample> &imu,
             double timestamp);
  ...
  void searchMapPoints(const std::vector<Descriptor> &descriptors,
                       const std::vector<KeyPoint> &kps,
                       std::vector<std::pair<int, int>> &matches);

  bool relocalize(const std::vector<Descriptor> &descriptors,
                  const std::vector<KeyPoint> &kps);
};
